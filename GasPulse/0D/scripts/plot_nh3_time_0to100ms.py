"""Plot the validated 100 ms NH3 endpoint trajectory for the square-wave case."""
...
SERIES_PATH = (
    CASE_DIR / "time_scan_rescue_20260912" / "100ms" / "attempt_02" / "results"
    / "pulse_series_rescue_t100ms.csv"
)
OUTPUT_PATH = CASE_DIR / "analysis" / "nh3_concentration_vs_time_0to100ms.png"
EXPECTED_FINAL_NH3 = 1.68116e15
...
def main() -> None:
    if not SERIES_PATH.is_file():
        raise FileNotFoundError(f"Missing 100 ms trajectory: {SERIES_PATH}")

    time_ms, nh3_cm3 = load_off_phase_endpoints(SERIES_PATH)
    if abs(time_ms[-1] - 100.0) > 1.0e-9:
        raise RuntimeError("The input trajectory does not reach 100 ms")
    if abs(nh3_cm3[-1] - EXPECTED_FINAL_NH3) / EXPECTED_FINAL_NH3 > 1.0e-6:
        raise RuntimeError("Unexpected 100 ms NH3 endpoint")
    ...
    axes[0].annotate(
        r"100 ms: $1.681\times10^{15}$ cm$^{-3}$",
        xy=(time_ms[-1], nh3_cm3[-1]), xytext=(57, 1.25e15),
