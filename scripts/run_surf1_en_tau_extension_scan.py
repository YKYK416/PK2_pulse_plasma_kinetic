"""Extend the Surf1 E/N versus residence-time scan beyond the prior boundary."""
...
EN_VALUES_TD = (95.0, 100.0, 105.0, 110.0, 115.0, 120.0)
TAU_VALUES_MS = (8.0, 10.0, 12.0, 15.0, 20.0)


def _write_case_config(label: str, en_td: float, tau_ms: float) -> tuple[str, Path]:
    case_id = f"surf1-cstr0d-extend-{_slug(label)}"
    ...
    data: dict[str, Any] = {
        ...
        "n_sub_on": 240,
        "n_sub_off": 160,
        "notes": "Surf1 extended E/N-residence-time scan at a 1000 ms horizon.",
    }
...
def main() -> int:
    ...
    specs = [
        (f"EN{en_td:g}Td_tau{tau_ms:g}ms", {"EN_on_Td": en_td, "residence_time_ms": tau_ms})
        for en_td in EN_VALUES_TD
        for tau_ms in TAU_VALUES_MS
    ]
    cases = [
        (*_write_case_config(label, params["EN_on_Td"], params["residence_time_ms"]), params)
    ...
    metadata = {
        ...
        "EN_values_Td": EN_VALUES_TD,
        "residence_times_ms": TAU_VALUES_MS,
        "cases": [
    ...
    with ThreadPoolExecutor(max_workers=min(args.max_workers, len(cases))) as executor:
        futures = {
            executor.submit(_run_case, case_id, "EN_tau_extension", params["residence_time_ms"], config_path, output_root): (case_id, params)
            for case_id, config_path, params in cases
        }
