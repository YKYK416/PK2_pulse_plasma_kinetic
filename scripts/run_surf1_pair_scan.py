"""Run stable second-stage Surf1 two-factor interaction scans."""
...
def _pair_specs() -> list[tuple[str, dict[str, Any], dict[str, Any]]]:
    ...
    def add(
        ...
        parameters = {"pair_family": family, **parameters, "site_density_factor": 1.0}
        specs.append((f"{family}-{label}", parameters, overrides))

    # E/N x residence time, with ne, T, and feed composition at baseline.
    for en_td in (50.0, 75.0, 100.0):
        for tau_ms in (1.0, 3.0, 10.0):
            add(
                "EN_tau",
                f"EN{en_td:g}Td_tau{tau_ms:g}ms",
                {"EN_on_Td": en_td, "residence_time_ms": tau_ms},
                {"EN_on_Td": en_td, "reactor": {"residence_time_s": tau_ms * 1.0e-3}},
            )

    # Electron density x residence time, with E/N, T, and feed composition at baseline.
    for ne_cm3 in (1.17e8, 3.0e8, 1.0e9):
        for tau_ms in (1.0, 3.0, 10.0):
            add(
                "ne_tau",
                f"ne{ne_cm3:.3e}_tau{tau_ms:g}ms",
                {"electron_density_cm3": ne_cm3, "residence_time_ms": tau_ms},
                {
                    "initial_state": {"electron_density_cm3": ne_cm3},
                    "feed": {"electron_density_cm3": ne_cm3},
                    "reactor": {"residence_time_s": tau_ms * 1.0e-3},
                },
            )

    # Gas temperature x H2 fraction, with E/N, ne, and residence time at baseline.
    for temperature_k in (350.0, 400.0, 500.0):
        for x_h2 in (0.50, 0.65, 0.80):
            add(
                "T_xH2",
                f"T{temperature_k:g}K_xH2{x_h2:g}",
                {"gas_temperature_K": temperature_k, "x_H2": x_h2},
                {
                    "initial_state": {
...
def _write_case_config(label: str, overrides: dict[str, Any]) -> tuple[str, Path]:
    case_id = f"surf1-cstr0d-pair-{_slug(label)}"
    ...
    data: dict[str, Any] = {
        ...
        "n_sub_on": 240,
        "n_sub_off": 160,
        "notes": "Surf1 second-stage two-factor interaction scan at a 1000 ms horizon.",
    }
...
def _write_summary(output_root: Path, summaries: list[dict[str, Any]]) -> None:
    summaries = sorted(summaries, key=lambda item: (item["pair_family"], item["case_id"]))
...
def main() -> int:
    ...
    with ThreadPoolExecutor(max_workers=min(args.max_workers, len(cases))) as executor:
        futures = {
            executor.submit(_run_case, case_id, "pair", None, config_path, output_root): (case_id, parameters)
            for case_id, config_path, parameters in cases
        }
